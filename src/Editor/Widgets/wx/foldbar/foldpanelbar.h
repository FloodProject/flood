...
public:
    ...
    */
	...
	void SetIcon(wxImage* image)
	{
		...
		style.SetExtraIcon(image);
		GetItem()->ApplyCaptionStyle(style);
	};
};
...
public:
    ...
    wxFoldPanel AddFoldPanel(const wxString &caption, int proportion = 0, bool collapsedInitially = false,
    ...
    /** Adds a wxWindow derived class to the referenced wxFoldPanel. IMPORTANT: Make the to be created window,
        ...
        \endcode
    */
    int AddFoldPanelWindow(const wxFoldPanel &panel, wxWindow *window, wxSizerFlags sizerFlags, int flags = wxFPB_ALIGN_WIDTH,
 ...
 /** Sets the style of the caption bar (called wxCaptionBar) of the wxFoldPanel. The changes are applied immediately.
    ...
    /** Removes all the fold panels from this bar. */
    void RemoveFoldPanels() 
    {
		m_panelSizer->Clear();
		DestroyChildren();
		
		// Work around for when the panel is empty.
		// FitInside() will only work when it has children.
