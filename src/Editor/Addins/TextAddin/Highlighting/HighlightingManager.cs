				...
				try {
					...
					using (Stream s = Resources.OpenStream(resourceName)) {
						using (XmlTextReader reader = new XmlTextReader(s)) {
							xshd = Highlighting.Xshd.HighlightingLoader.LoadXshd(reader, false);
						}
					}
...
//					}
					...
					RegisterHighlighting(name, extensions, Highlighting.Xshd.HighlightingLoader.Load(xshd, this));
					#else
				...
				Func<IHighlightingDefinition> func = delegate {
					...
					using (Stream s = Resources.OpenStream(resourceName)) {
						using (XmlTextReader reader = new XmlTextReader(s)) {
							...
							xshd = Highlighting.Xshd.HighlightingLoader.LoadXshd(reader, true);
						}
					}
					return Highlighting.Xshd.HighlightingLoader.Load(xshd, this);
				};
				return func;
			}
		}
