	...
	sealed class V1Loader
	{
		...
		static XshdColor GetColorFromElement(XmlElement element)
		{
			...
			if (element.HasAttribute("color"))
				color.Foreground = ColorConverter.ConvertFromString(element.GetAttribute("color"));
			if (element.HasAttribute("bgcolor"))
				color.Background = ColorConverter.ConvertFromString(element.GetAttribute("bgcolor"));
			return color;
		}
