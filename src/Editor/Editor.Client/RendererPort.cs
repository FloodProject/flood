...
using StringTextureCache = System.Collections.Generic.Dictionary<System.Tuple<System.String, Flood.GUI.Font>, Flood.GUI.Texture>;
...
class ManagedGeometryBuffer {
    ...
    //HandleId,MaterialHandle
    Dictionary<uint,Material> materials;
    ...
    Material GetCreateMaterial(uint hId)
    {
        if (!materials.ContainsKey(hId))
        {
            ...
            materials.Add(hId,mat);
        }
        return materials[hId];
    }

    BatchInfo GetCreateBatchInfo(uint hId)
    {
    ...
    public void addRectangle(System.Drawing.Rectangle rect, Vector2 uv1, Vector2 uv2, uint imageHandle, System.Drawing.Color color)
    {
        ...
        Color c = new Color(color.R/255.0f,color.G/255.0f,color.B/255.0f,color.A/255.0f);
    ...
    {
        foreach(BatchInfo bInfo in  batches.Values){

            if (bInfo.ranges.Count > 0)
            {
                bInfo.batch.Range.Start = (ushort) gb.GetNumIndices();
                for (var ir = 0; ir < bInfo.ranges.Count; ++ir){
                    ushort vertexIndex = (ushort)bInfo.ranges[ir];
                    gb.AddIndex(vertexIndex++);
                    gb.AddIndex(vertexIndex++);
                    gb.AddIndex(vertexIndex++);
                    gb.AddIndex(vertexIndex);
                }
                bInfo.batch.Range.End = (ushort)gb.GetNumIndices();

                var state = new RenderState(bInfo.batch);
...
public class NativeGUI {
    ...
    void Initialize(InputManager inputManager) {
        ...
        managedEditor = new Flood.Editor.Editor(renderer,"DefaultSkin.png");
...
static NativeGUI gs_GUIInstance = null;

void InitializeGUI(InputManager inputManager){
    gs_GUIInstance = new NativeGUI(inputManager);
}
