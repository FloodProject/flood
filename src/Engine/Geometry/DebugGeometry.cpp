    ...
    , triangles(nullptr)
{
    ...
    renderables.Push(triangles.get());
...
RenderablePtr DebugBuildRay( const Ray& pickRay, float length )
{
    ...
    vertex.Push( pickRay.origin );
    vertex.Push( pickRay.getPoint(length) );

    Vector<Vector3> colors( 2, Color::Red );
...
#define ADD_BOX_FRUSTUM( a, b, c, d ) \
    pos.Push( box.corners[a] ); \
    pos.Push( box.corners[b] ); \
    pos.Push( box.corners[c] ); \
    pos.Push( box.corners[d] );
