	...
	switch(event.Type)
	{
		case sf::Event::MouseButtonPressed:
		{
			MouseButtonEvent mbe(event.MouseButton.X, event.MouseButton.Y, 
				convertMouseButtonEnum(event.MouseButton.Button), MouseEventType::MousePress);
			processEvent(mbe);
		}	
		case sf::Event::MouseButtonReleased:
		{
			MouseButtonEvent mbe(event.MouseButton.X, event.MouseButton.Y, 
				convertMouseButtonEnum(event.MouseButton.Button), MouseEventType::MouseRelease);
			processEvent(mbe);
		}
...
void SFML_InputManager::processKeyboardEvent(const sf::Event& event)
{
	switch(event.Type)
	{
		...
		{
			KeyEvent ke(convertKeyEnum(event.Key.Code), event.Key.Alt, event.Key.Shift, event.Key.Control,
...
void SFML_InputManager::processJoystickEvent(const sf::Event& event)
{
	switch(event.Type)
	{
		...
		{
			JoyButtonEvent jbe(event.JoyButton.JoystickId, event.JoyButton.Button, JoystickEventType::JoystickPress);
			processEvent(jbe);
		}
		case sf::Event::JoyButtonReleased:
		{
			JoyButtonEvent jbe(event.JoyButton.JoystickId, event.JoyButton.Button, JoystickEventType::JoystickRelease);
			processEvent(jbe);
		}
		case sf::Event::JoyMoved:
		{
			JoyMoveEvent jme(event.JoyMove.JoystickId, convertJoystickAxisEnum(event.JoyMove.Axis), event.JoyMove.Position);
			processEvent(jme);
		}
