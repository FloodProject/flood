...
public:
    ...
    void operator=(const wxCaptionBarStyle &s) {
        ...
        if(s.m_extraIconUsed)
        {
            m_extraIconUsed = true;
            m_extraIcon = s.m_extraIcon;
        }
		if(s.m_visible)
		{
    ...
    /** Set extra icon for the caption bar. If this is not set, the icon property is
        undefined and will not be used. Use ExtraIconUsed() to check if this style is used */
    void SetExtraIcon(wxImage* image) {
        if(image != NULL)
        {
          m_extraIcon = wxBitmap(*image);
          m_extraIconUsed = true;
        }
        else
        {
          m_extraIcon = wxBitmap();
          m_extraIconUsed = false;
        }
    };

    /** Checks if the extra icon of the caption bar is set */
    bool ExtraIconUsed() const {
        return m_extraIconUsed;
    };

    /** Returns the extra icon for the caption bar. Please be warned this will
        result in an assertion failure when this property is not previously set.
        \sa SetExtraIcon(), ExtraIconUsed() */
    const wxBitmap& GetExtraIcon() const {
        wxASSERT(m_extraIconUsed);
        return m_extraIcon;
    };    
...
private:
    ...
    /* Computes the rect of the icon */
    void ComputeIconRect(wxRect wndRect, wxRect& rect);
